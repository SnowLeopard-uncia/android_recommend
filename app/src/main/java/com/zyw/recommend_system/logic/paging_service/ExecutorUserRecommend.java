...
//目前想法是每个接口都搞一个接口实现类
public class ExecutorUserRecommend implements LoadDataInterface<Article> {
    ...
    public ExecutorUserRecommend(ListeningExecutorService executorService,boolean isFirst,String tag) {
    ...
    public ListenableFuture<List<Article>> load(int pageNum) {
        return executorService.submit(() -> {
            if (isFirst){
                Call<BaseResponse<ArticleList>> call = articleService.getFirstRecommendArticleList(tag,"15",Integer.toString(pageNum));
                ...
                //  耗时操作 请求列表
                Call<BaseResponse<ArticleList>> call = articleService.getRecommendArticleList("15",Integer.toString(pageNum));
                BaseResponse<ArticleList> articleListResponse = call.execute().body();
                List<Article> list = articleListResponse.getData().getArticleList();
                return list;
            }
